...
Use this template to build the prompt for the explainer `task` subagent. Fill in the placeholders.
...
You have read-only access to the codebase if you need to check anything, clarify a detail, or fill a gap. Use `read` to examine files, `bash` with `rg` to search for symbols, and `finder` for semantic code search — but the explorers already did the heavy lifting, so you shouldn't need to re-explore from scratch.

...
Use this template to build the prompt for each critic `task` subagent. Fill in the placeholders.
...
## Your Primary Analytical Focus

{ANALYTICAL_FOCUS}

You may comment on any issue you find, but go deepest on your assigned lenses above.
...
Read the files listed above. Use `read` to examine files and `bash` with `rg` to search for symbols. Use the explanation as a map, but form your own opinions from the code itself — the explanation might miss things or frame them charitably.

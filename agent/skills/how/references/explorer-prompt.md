...
Use this template to build the prompt for the explorer `task` subagent. Fill in the placeholders.
...
Start by finding the relevant code. Use `bash` with `find` or `rg` (ripgrep) to find directories and files, `rg` to find key symbols, `read` to understand the actual implementation, and `finder` for semantic code search when you need to locate concepts across the codebase. Don't guess from names — read the code.

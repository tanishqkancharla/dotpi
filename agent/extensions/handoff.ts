 ...
 *
 * Usage:
 *   /handoff                              (opens input overlay, ctrl+space cycles mode)
...
import { applyMode } from "./amp/modes.js";
import { promptForTextWithMode } from "./lib/mode-prompt.js";
...
async function applyHandoffOptions(
  ...
  if (options?.mode) {
    await applyMode(pi, ctx, options.mode);
  }
}
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("context", (event) => {
    ...
    const newMessages = event.messages.filter(
      (m: any) => m.timestamp >= handoffTimestamp,
    );
  ...
  pi.registerCommand("handoff", {
    description: "Transfer context to a new focused session (-mode <name>)",
    handler: async (args, ctx) => {
      // Parse optional -mode flag from args
      ...
      if (!goal) {
        const prompt = await promptForTextWithMode(ctx, {
          label: "handoff",
          placeholder: "Describe what the next session should do...",
        });
        if (!prompt) return;
        goal = prompt.text;
        if (prompt.modeName) {
          options.mode = prompt.modeName;
        }
      }

      const hasOptions = !!options.mode;
  ...
  pi.registerTool({
    name: "handoff",
    label: "Handoff",
    description: "Hand off work to a new session.",
    parameters: Type.Object({
      goal: Type.String({ description: "Description of the next task" }),
      mode: Type.Optional(
        Type.String({ description: "Agent mode for the new session" }),
      ),
    }),
